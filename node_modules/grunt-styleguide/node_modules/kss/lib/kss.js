...
parseChunk = function(data, input, options) {
	...
	for (i = 0; i < l; i += 1) {
		...
		currSection = {
			...
			modifiers: [],
			markup: false,
			responsiveMarkup: false,
			jsIncludes: false
		};
		//console.log("p^^^^^^",currSection.raw);
		...
		//coonsole.log("raw********", currSection.raw);
...
processMarkup = function(paragraphs, options, sectionData) {
	...
	paragraphs.map(function(paragraph, index) {
		...
		if (hasPrefix(paragraph, options, 'Markup')) {
			...
			//console.log("markup======",sectionData.markup);
		}

		if (hasPrefix(paragraph, options, 'ResponsiveMarkup')) {
			sectionData.markup = paragraph.replace(/ResponsiveMarkup\:\s+?/i, '');
			//console.log("markdup --- ",sectionData.markup);
			sectionData.responsiveMarkup =  true;
			paragraph = '';
			removeIndex = index;

			//console.log("responsivemarkup======",sectionData.responsivemarkup);
		}

		if (hasPrefix(paragraph, options, 'js-includes')) {
			sectionData.jsIncludes = paragraph.replace(/js-includes\:\s+?/i, '');

			console.log("js includes -----",sectionData.jsIncludes);
		}
